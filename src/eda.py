...
ax.grid(True, linestyle="--", alpha=0.4)
plt.tight_layout()
plt.savefig(output_dir / "eda_boxplot_risco_mes.png", dpi=150)
plt.close()
print("OK")
...
# ====== Boxplot: Dias sem Chuva por Bioma ====================================

print("Gerando boxplot dias sem chuva por bioma... ", end="", flush=True)
...
ax.set_title("Dias sem Chuva por Bioma — Brasil 2024", fontsize=13)
ax.set_xlabel("Bioma")
ax.set_ylabel("Número de Dias sem Chuva")
plt.suptitle("")
ax.grid(True, linestyle="--", alpha=0.4)
plt.tight_layout()
plt.savefig(output_dir / "eda_boxplot_dias_sem_chuva_bioma.png", dpi=150)
plt.close()
print("OK")

print(f"Gráficos salvos em: {output_dir}")

# ====== Scatter: Dias sem Chuva x Risco de Fogo por Bioma ===================

print("Gerando scatter dias sem chuva x risco de fogo por bioma... ", end="", flush=True)

# Amostra estratificada por bioma e mês — 100 registros por grupo
df_scatter = (
    df.groupby(["bioma", "mes"], group_keys=False)
...
for bioma, grupo in df_scatter.groupby("bioma"):
    ax.scatter(
        grupo["numero_dias_sem_chuva"],
        grupo["risco_fogo"],
        label=bioma,
        color=cores.get(bioma, "gray"),
        alpha=0.5,
        s=15
    )

ax.set_title("Dias sem Chuva vs Risco de Fogo por Bioma — Brasil 2024", fontsize=13)
ax.set_xlabel("Número de Dias sem Chuva")
ax.set_ylabel("Risco de Fogo (0-1)")
ax.legend(title="Bioma")
ax.grid(True, linestyle="--", alpha=0.4)
plt.tight_layout()
plt.savefig(output_dir / "scatter_dias_chuva_risco_bioma.png", dpi=150)
plt.close()
print("OK")
...
pivot.columns = [meses_label[m] for m in pivot.columns]
...
fig, ax = plt.subplots(figsize=(12, 6))
focos_bioma_mes.plot.area(
    ax=ax,
    color=[cores.get(b, "gray") for b in focos_bioma_mes.columns],
    alpha=0.7
)
...
for bioma in dias_bioma_mes.columns:
    ax.plot(
        dias_bioma_mes.index,
        dias_bioma_mes[bioma],
        marker="o",
        label=bioma,
        color=cores.get(bioma, "gray"),
        linewidth=2
